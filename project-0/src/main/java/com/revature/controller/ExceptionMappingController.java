...
public class ExceptionMappingController {
	...
	public void mapExceptions(Javalin app) {
		...
		// For example, in the cases where we want to modify or delete or get a client who does not exist (PUT /clients/10000, DELETE/clients/1000,
		// GET /clients/10000), that should result in some kind of ClientNotFoundException being thrown in the service layer. 
		// When it propagates to the controller layer, that is when we can go ahead and specify a 404 Not found HTTP status code (and an appropriate
		// exception message that is encapsulated via a DTO (ExceptionMessageDTO))
